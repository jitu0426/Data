...
def generate_story_html(story_img_b64: str) -> str:
    ...
    img_tag = (
        ...
        f'style="max-width:100%;height:auto;border:1px solid #e8870a;border-radius:6px;" />'
        if story_img_b64
        else '<div style="padding:40px;border:2px dashed #e8870a;color:#e8870a;text-align:center;">Image not found</div>'
    )
    return f"""
    <div class="story-page" style="page-break-after:always;padding:25px 50px;
         font-family:sans-serif;background:#ffffff;color:#3d2020;">
      <h1 style="text-align:center;color:#c8102e;font-size:28pt;margin-bottom:16px;
                 letter-spacing:3px;text-transform:uppercase;">Our Journey</h1>
      <div style="height:2px;background:linear-gradient(90deg,transparent,#c8102e,#e8870a,#c8102e,transparent);margin-bottom:24px;"></div>
      <p style="font-size:11pt;line-height:1.7;margin-bottom:20px;text-align:justify;color:#3d2020;">{text1}</p>
      <div style="overflow:auto;margin-bottom:20px;">
        <div style="float:left;width:50%;padding-right:20px;font-size:11pt;line-height:1.7;text-align:justify;color:#3d2020;">{text2}</div>
        <div style="float:right;width:46%;text-align:center;">{img_tag}</div>
      </div>
      <div style="clear:both;"></div>
      <h2 style="text-align:center;font-size:13pt;color:#e8870a;margin-top:24px;letter-spacing:2px;">
...
def generate_table_of_contents_html(df_sorted: pd.DataFrame) -> str:
    ...
    css = """
    <style>
      .toc-title{text-align:center;font-family:serif;font-size:30pt;color:#c8102e;
                 margin:10px 0 6px;text-transform:uppercase;letter-spacing:3px;}
      .toc-gold-line{height:2px;background:linear-gradient(90deg,transparent,#c8102e,#e8870a,#c8102e,transparent);
                     margin:0 auto 20px;width:60%;}
      h3.toc-cat-header{background:linear-gradient(135deg,#fdedf0,#fff5e6);
                        color:#c8102e;font-family:sans-serif;font-size:14pt;
                        padding:10px 16px;margin:0 0 14px;border-left:5px solid #c8102e;
      ...
      a.idx-card{display:inline-block;width:30%;margin:1.5%;height:195px;
                 background:#ffffff;border-radius:8px;text-decoration:none;
                 overflow:hidden;border:1px solid rgba(200,16,46,0.20);
                 page-break-inside:avoid;vertical-align:top;}
      .idx-img{width:100%;height:155px;background-repeat:no-repeat;
               background-position:center;background-size:contain;background-color:#fff9f5;}
      .idx-label{height:40px;background:linear-gradient(135deg,#c8102e,#8b0a1e);
                 color:#ffffff;font-family:sans-serif;font-size:8.5pt;font-weight:800;
    ...
    <div id="main-index" class="toc-page"
         style="page-break-after:always;padding:20px;background:#fff9f5;">
    ...
    for cat_name in catalogues:
        pb = "" if first else 'style="page-break-before:always;padding-top:20px;background:#fff9f5;"'
        css += f'<div {pb}>'
        ...
        for category in cat_df["Category"].unique():
            ...
            for _, row in grp.iterrows():
                ...
                if len(s) > 100:
                    rep_img = s
                    break
            bg = f"background-image:url('data:image/png;base64,{rep_img}');" if rep_img else "background-color:#fff9f5;"
...
html,body{{margin:0!important;padding:0!important;width:100%!important;
           background:#fff9f5!important;color:#3d2020;}}
...
.cat-content{{padding:8mm 10mm 50px;position:relative;z-index:1;background:#fff9f5;}}
.cat-heading{{background:linear-gradient(135deg,#fdedf0,#fff5e6);
              color:#c8102e;font-size:16pt;padding:10px 18px;
              margin-bottom:4px;font-weight:bold;font-family:sans-serif;
              border-left:5px solid #c8102e;page-break-inside:avoid;clear:both;}}
.category-heading{{color:#1a0a0a;font-size:13pt;padding:8px 0 4px;
                   border-bottom:1px solid rgba(200,16,46,0.30);
                   margin-top:6mm;clear:both;font-family:serif;
                   page-break-inside:avoid;width:100%;}}
.subcat-hdr{{color:#e8870a;font-size:10pt;font-weight:bold;margin:10px 0 4px;
             clear:both;font-family:sans-serif;border-left:2px solid #e8870a;
             padding-left:8px;page-break-inside:avoid;width:100%;
             background:#fff5e6;padding:4px 8px;border-radius:0 4px 4px 0;}}
.cs-table{{width:100%;border-collapse:collapse;font-size:8.5pt;
           margin-bottom:10px;clear:both;background:#ffffff;}}
.cs-table th{{border:1px solid rgba(200,16,46,0.25);background:#fdedf0;
              padding:4px;text-align:center;font-weight:bold;color:#c8102e;}}
.cs-table td{{border:1px solid rgba(0,0,0,0.10);padding:4px;
              text-align:center;color:#6b4040;}}
...
.prod-card{{display:inline-block;width:23%;margin:8px 1%;vertical-align:top;
            ...
            border:1px solid rgba(200,16,46,0.20);border-radius:6px;
            text-align:center;position:relative;overflow:hidden;
            height:178px;page-break-inside:avoid;
            box-shadow:0 1px 4px rgba(200,16,46,0.08);}}
.card-img{{width:100%;height:112px;position:relative;
           background-color:#fff9f5;border-bottom:1px solid rgba(200,16,46,0.12);
           overflow:hidden;}}
...
.card-name{{font-family:serif;color:#1a0a0a;line-height:1.2;
    ...
    for index, row in df_sorted.iterrows():
        ...
        if row["Category"] != cur_category:
            ...
            html_parts.append(
                ...
                f'<a href="#main-index" style="float:right;font-size:9px;color:#e8870a;'
            ...
            if row_data:
                ...
                if desc:
                    html_parts.append(
                        f'<p style="color:#6b4040;font-size:9.5pt;font-style:italic;margin:4px 0;">'
                        f'<strong style="color:#c8102e;">Case Size:</strong> {desc}</p>'
                    )
        ...
        img_html = (
            ...
            if img_b64
            else '<div style="padding-top:35px;color:#a07070;font-size:9px;">NO IMAGE</div>'
        )
        new_badge = (
        ...
        <div class="prod-card">
          ...
          <div class="card-info">
            <div class="card-name" style="font-size:{fs};">
              <span style="color:#c8102e;margin-right:2px;">{index+1}.</span>{name}
            </div>
          </div>
        </div>""")
