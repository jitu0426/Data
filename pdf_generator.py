...
PDF THEME: Plain white — no dark backgrounds, no gold.
  Backgrounds : #ffffff (pure white throughout)
  Body text   : #222222 (dark readable)
  Accent text : #c8102e (HEM Crimson Red — headings, borders, badges only)
  Grey text   : #333333 / #555555 / #888888
  Borders     : #dddddd / #e0e0e0 / #eeeeee (light grey)
  Table header: #f5f5f5 (near white)
  Image bg    : #f8f8f8 (near white)
"""
...
def generate_story_html(story_img_b64: str) -> str:
    """Build the 'Our Journey' page HTML — plain white background."""
    text1 = (
    ...
    img_tag = (
        ...
        f'style="max-width:100%;height:auto;border:1px solid #dddddd;border-radius:4px;" />'
        if story_img_b64
        else '<div style="padding:40px;border:1px dashed #cccccc;color:#999999;text-align:center;">Image not found</div>'
    )
    return f"""
    <div class="story-page" style="page-break-after:always;padding:25px 50px;
         font-family:sans-serif;background:#ffffff;color:#222222;">
      <h1 style="text-align:center;color:#c8102e;font-size:26pt;margin-bottom:12px;
                 letter-spacing:2px;text-transform:uppercase;">Our Journey</h1>
      <div style="height:1px;background:#dddddd;margin-bottom:22px;"></div>
      <p style="font-size:11pt;line-height:1.8;margin-bottom:18px;text-align:justify;color:#333333;">{text1}</p>
      <div style="overflow:auto;margin-bottom:20px;">
        <div style="float:left;width:50%;padding-right:20px;font-size:11pt;line-height:1.8;text-align:justify;color:#333333;">{text2}</div>
        <div style="float:right;width:46%;text-align:center;">{img_tag}</div>
      </div>
      <div style="clear:both;"></div>
      <div style="height:1px;background:#dddddd;margin-top:22px;margin-bottom:14px;"></div>
      <h2 style="text-align:center;font-size:11pt;color:#c8102e;margin-top:0;letter-spacing:3px;font-weight:600;">
...
def generate_table_of_contents_html(df_sorted: pd.DataFrame) -> str:
    """Generate the Table of Contents with category thumbnail cards — plain white."""
    css = """
    <style>
      .toc-title{text-align:center;font-family:serif;font-size:28pt;color:#c8102e;
                 margin:10px 0 6px;text-transform:uppercase;letter-spacing:2px;}
      .toc-divider{height:1px;background:#dddddd;margin:0 auto 20px;width:70%;}
      h3.toc-cat-header{background:#ffffff;color:#222222;font-family:sans-serif;font-size:13pt;
                        padding:8px 14px;margin:0 0 12px;border-left:3px solid #c8102e;
                        border-bottom:1px solid #eeeeee;page-break-inside:avoid;}
      .idx-grid{display:block;width:100%;font-size:0;}
      a.idx-card{display:inline-block;width:30%;margin:1.5%;height:195px;
                 background:#ffffff;border-radius:6px;text-decoration:none;
                 overflow:hidden;border:1px solid #e0e0e0;
                 page-break-inside:avoid;vertical-align:top;}
      .idx-img{width:100%;height:155px;background-repeat:no-repeat;
               background-position:center;background-size:contain;background-color:#f8f8f8;}
      .idx-label{height:40px;background:#c8102e;
                 color:#ffffff;font-family:sans-serif;font-size:8.5pt;font-weight:700;
    ...
    <div id="main-index" class="toc-page"
         style="page-break-after:always;padding:20px;background:#ffffff;">
    ...
    for cat_name in catalogues:
        pb = "" if first else 'style="page-break-before:always;padding-top:20px;background:#ffffff;"'
        css += f'<div {pb}>'
        ...
        for category in cat_df["Category"].unique():
            ...
            for _, row in grp.iterrows():
                ...
                if len(s) > 100:
                    rep_img = s
                    break
            bg = f"background-image:url('data:image/png;base64,{rep_img}');" if rep_img else "background-color:#f8f8f8;"
...
def generate_pdf_html(df_sorted: pd.DataFrame, customer_name: str,
    ...
    """Assemble complete HTML: Cover → Story → TOC → Product pages. Plain white theme."""
...
.cat-content{{padding:8mm 10mm 50px;position:relative;z-index:1;background:#ffffff;}}
.cat-heading{{background:#ffffff;color:#c8102e;font-size:15pt;padding:8px 14px;
              margin-bottom:6px;font-weight:bold;font-family:sans-serif;
              border-left:4px solid #c8102e;border-bottom:1px solid #eeeeee;
              page-break-inside:avoid;clear:both;}}
.category-heading{{color:#222222;font-size:12pt;padding:7px 0 5px;
                   border-bottom:1px solid #dddddd;
                   margin-top:6mm;clear:both;font-family:serif;
                   page-break-inside:avoid;width:100%;}}
.subcat-hdr{{color:#555555;font-size:9.5pt;font-weight:bold;margin:8px 0 4px;
             clear:both;font-family:sans-serif;border-left:2px solid #c8102e;
             padding:3px 8px;page-break-inside:avoid;width:100%;
             background:#ffffff;}}
.cs-table{{width:100%;border-collapse:collapse;font-size:8.5pt;
           margin-bottom:10px;clear:both;background:#ffffff;}}
.cs-table th{{border:1px solid #dddddd;background:#f5f5f5;
              padding:4px;text-align:center;font-weight:bold;color:#333333;}}
.cs-table td{{border:1px solid #e8e8e8;padding:4px;
...
.prod-card{{display:inline-block;width:23%;margin:8px 1%;vertical-align:top;
            font-size:12pt;padding:0;background:#ffffff;
...
.card-name{{font-family:serif;color:#222222;line-height:1.2;
            font-weight:bold;margin:0;padding-top:4px;display:block;}}
.new-badge{{position:absolute;top:0;right:0;background:#c8102e;color:white;
            font-size:7px;font-weight:bold;padding:2px 7px;
            border-radius:0 0 0 4px;z-index:10;letter-spacing:0.5px;}}
.clearfix::after{{content:"";clear:both;display:table;}}
    ...
    for index, row in df_sorted.iterrows():
        ...
        if row["Category"] != cur_category:
            ...
            html_parts.append(
                ...
                f'<a href="#main-index" style="float:right;font-size:9px;color:#888888;'
            ...
            if row_data:
                ...
                if desc:
                    html_parts.append(
                        f'<p style="color:#555555;font-size:9.5pt;font-style:italic;margin:4px 0;">'
                        f'<strong style="color:#333333;">Case Size:</strong> {desc}</p>'
                    )
        ...
        img_html = (
            ...
            if img_b64
            else '<div style="padding-top:35px;color:#bbbbbb;font-size:9px;">NO IMAGE</div>'
        )
        new_badge = (
        ...
        <div class="prod-card">
          ...
          <div class="card-info">
            <div class="card-name" style="font-size:{fs};">
              <span style="color:#888888;margin-right:2px;">{index+1}.</span>{name}
            </div>
          </div>
        </div>""")
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        for r, lbl in [(3, "20 FT (30 CBM)"), (4, "40 FT (60 CBM)"), (5, "40 FT HC (70 CBM)")]:
            ws.write(r-1, 1, "CONTAINER TYPE",
                     wb.add_format({"bold": True, "bg_color": "#c8102e",
                                     "font_color": "#ffffff", "border": 1})) if r == 3 else None
